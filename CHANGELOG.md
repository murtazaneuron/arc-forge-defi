...
All notable changes to `arc-forge-defi` are documented here.
...
- **`.clippy.toml`** — corrected project header from `hft-crypto`
  to `arc-forge-defi`; updated `msrv` to `"1.85.0"` to match
  the corrected `Cargo.toml` `rust-version`
- **`rustfmt.toml`** — corrected project header from `hft-crypto`
  to `arc-forge-defi`
...
- `.gitignore` - consolidated with focused Rust-only ignore rules matching `hft-crypto`
...
- `arc-forge-defi` CLI: validate, raydium, launch, agent, check
